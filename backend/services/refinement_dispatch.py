"""Refinement dispatch + worker service.

Owns the full refinement execution path: `dispatch_refinement_for_job`
(create/reset state + submit), the `_run_refinement_for_job` worker, its
`_run_post_refinement_learning` (B7) tail, the `_cleanup_deferred_audio` helper,
and the shared `_set_refinement_status`. Relocated out of `routes/refinement.py`
(S5 moved dispatch; the F1 follow-up moved the workers) so neither the
orchestrator nor this service imports a route — the service is self-contained
and `routes/refinement.py` re-exports these for its endpoint wrappers and tests.

Import discipline: imports `state`/`app_state` and (lazily, as the original
route did) `services.transcription`/`services.glossary`/`services.learning`.
It must NOT import anything from `routes`.
"""
...
def dispatch_refinement_for_job(
    ...
    # F1: the worker now lives in this service, so submit it directly — no
    # route import, no cycle.
